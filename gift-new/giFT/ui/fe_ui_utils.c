...
{
	Transfer	 *item;
	GList		 *sel, *l_rem = NULL;

	assert (ctree);
	for (sel = GTK_CLIST (ctree)->selection; sel; sel = sel->next)
	{
		item = gtk_ctree_node_get_row_data (GTK_CTREE (ctree), sel->data);
		assert (item);

		/* only work with top nodes of multiple source downloads */
		if (GTK_CTREE_ROW (sel->data)->level == 1)
			l_rem = g_list_append (l_rem, item);
	}

	/* now call the function on them */
	for (sel = l_rem; sel ; sel = sel->next) {
		item = (Transfer *)sel->data;
		(*function) (item);
	}
	g_list_free (l_rem);
...
#if 0
	/* this logic appears to be flawed - hard code \n's for now -Ross */
