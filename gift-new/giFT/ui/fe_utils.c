...
static int generic_column_sort_int (GtkCList *clist, const void *p1,
                                    const void *p2);
static int generic_column_sort_double (GtkCList *clist,
...
char *calculate_transfer_info (size_t transmit, size_t total,
                               size_t diff)
{
	char *info, *l_trans, *l_tot;

	l_trans = format_size_disp (0, transmit);
	l_tot   = format_size_disp (0, total);
...
{
	float human = (float) size;
	/* kilo, mega, giga, tera, peta, exa, zetta, yotta. Source:
...
void generic_column_sort (GtkWidget *w, int column, char *sort_info)
{
	...
	switch (sort_info[column])
	{
	 ...
	 case 'i':
		...
		break;
	 case 'd':
		sort_func = generic_column_sort_double;
		break;
	 case ' ':
	 default:
		sort_func = NULL;
