...
array (
 "HOME" =>
    array ("main"               => "$prefix/"),
 "DEV" =>
    array ("main"               => "$prefix/dev/",
           "todo"               => "todo.php",
           "clients"            => "clients.php"),
 "DOWNLOAD" =>
    array ("main"               => "$prefix/download/",
           "most recent"        => "cvs.php",
           "old releases"       => "old.php"),
 "DOCUMENTATION" =>
    array ("main"               => "$prefix/docs/",
           "faq"                => "?document=faq.html",
           "installation guide" => "?document=install.html",
           "interface protocol" => "?document=interface.html",
           "whatis"             => "?document=whatis.html"),
 "NEWS" =>
...
function layout_header ($section, $page, $page_desc)
{
	global $view;
	global $prefix;
	...
	for ($i = 0; $i < count($sec); $i++)
	{
		...
		if ($section == $sec[$i])
		{
			if ($page != $filename[main])
				echo "<a class=\"current\" href=\"$filename[main]$viewurl\">$sec[$i]</a>";
			else
				echo "<span class=\"current\">$sec[$i]</span>";
		...
		{
			echo "<a href=\"$filename[main]$viewurl\">$sec[$i]</a>";
		}
