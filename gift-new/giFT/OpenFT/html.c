...
static int build_shares (FileShare *file, FILE *f)
{
	...
	Metadata *bitrate, *duration, *width, *height, *mime;
...
#if 0
	bitrate = meta_lookup(file->meta,TAG_BITRATE,NULL);
	duration = meta_lookup(file->meta,TAG_DURATION,NULL);
	width = meta_lookup(file->meta,TAG_WIDTH,NULL);
	height = meta_lookup(file->meta,TAG_HEIGHT,NULL);
	mime = meta_lookup(file->meta,TAG_MIME,NULL);
#endif
	...
	if (width && height)
		fprintf (f, "<TD>%lu &#215; %lu</TD>\n",
			 meta_get_integer(width),meta_get_integer(height));
	else
		fprintf (f, "<TD></TD>\n");

	if (mime)
		fprintf (f, "<TD>%s</TD>\n",
			 meta_get_string(mime));
	else
		fprintf (f, "<TD></TD>\n");
#endif	  
	fprintf (f, "</TR>\n");
...
char *html_page_index (char *file)
{
	...
	fprintf (f,
	         ...
	         "<h2>Shares</h2>\n"
	         "<table border='0' cellspacing='1' cellpadding='2'>\n"
	         "<tr><th><b>Filesize</b></th><th><b>Filename</b></th><th><b>Bitrate</b></th><th><b>Duration</b></th><th><b>Dimensions</b></th><th><b>MIME</b></th></tr>\n");
