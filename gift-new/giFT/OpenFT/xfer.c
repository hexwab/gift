...
int ft_transfer_set_request (FT_Transfer *xfer, char *request)
{
	...
	if ((file = share_find_file (shared_path)) && file->sdata)
	{
		...
		xfer->content_type = NULL;//meta_get_string (meta_lookup (file->meta, TAG_MIME, NULL));
	}
...
FILE *ft_transfer_open_request (FT_Transfer *xfer, int *code)
{
	...
	else
	{
		 ...
		 * table for a match */
		full_path = upload_auth (net_ip_str (xfer->ip), shared_path, &reason);
	                     ...
	                     (off_t *) &xfer->open_path_size)))
	{
		...
		/* this share could not be opened locally...automatically remove it
		 * from the list so that we dont annoy people trying to access it */
		share_remove_entry (host_path);
...
static void download_start (Chunk *chunk, void *data)
{
		...
		)
	{
		...
		else
		{
			 ...
			 * from */
			ft_packet_send_indirect (search_ip, FT_PUSH_RESPONSE, "+I%hu%s%lu%lu",
