                 ...
                 size_t kbps_min, size_t kbps_max)
{
	...
	if (local && mdata.size < max_results)
	{
		...
		mdata.max_uploads = upload_status ();
...
void search_reply (IFEventID id, Connection *search_node, FileShare *file)
{
	...
	if (!file)
	{
		search->ref = list_remove (search->ref, I_PTR (NODE (search_node)->ip));
...
{
	if (rem)
		hash_table_foreach_remove (active_searches, func, udata);
	else
		hash_table_foreach (active_searches, func, udata);
}
