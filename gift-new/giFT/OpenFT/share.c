...
static FT_HostShare *local_host_share = NULL;
...
static FT_Share *ft_share_new_proto (FileShare *file, FT_HostShare *h_share)
{
	...
	if (!(share = malloc (sizeof (FT_Share))))
		return NULL;
                         ...
                         char *md5, char *filename)
{
	...
	file = share_new (openft_proto, FALSE, NULL, 0, filename, md5, size, 0);
...
static void ft_share_free_proto (FileShare *file)
{
	...
	if (!file || !(share = share_lookup_data (file, "OpenFT")))
		return;
...
{
	/* ft_share_free_proto will be called through the grape vine.  see
	 * protocol_send if you don't believe me */
	share_free (file);
}
...
static int destroy_host_table (unsigned long key, FileShare *file, int *force)
{
	...
	if (share_unref (file))
	{
		/* didn't free, apply further cleanup */
		if (!(share = share_lookup_data (file, "OpenFT")))
			return TRUE;

		/* don't reference the host share anymore, it's gone */
		share->host_share = NULL;
	}
...
void ft_share_local_add (FileShare *file)
{
	...
	{
		local_host_share =
		    ft_host_share_new (TRUE, 0, NODE (ft_self)->port,
		                       NODE (ft_self)->http_port);
	}

	ft_share_new_proto (file, local_host_share);
}
...
void ft_share_local_submit (Connection *c)
{
	...
	{
		GIFT_ERROR (("Can't open %s: %s",
					share_comp_path (comp_req, &use_comp), GIFT_STRERROR ()));
		return;
	}
