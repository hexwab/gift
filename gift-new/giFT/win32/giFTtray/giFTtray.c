...
static int send_gift_command (char *event, ...)
{
	char *packet;
	va_list args;

	va_start (args, event);
	packet = interface_construct_packet (NULL, event, args);
	va_end (args);

	write_list = list_append (write_list, strdup (packet));
...
static int create_process (char *cmd_line, PROCESS_INFORMATION *proc_info)
{
	...
	if (!CreateProcess (NULL, cmd_line, NULL, NULL, TRUE, 0,
...
static int gift_attach (char *ip, unsigned short port)
{
	...
	WSAAsyncSelect (gift_conn->fd, app_window, MY_WM_SOCKET_EVENT,
	                FD_READ | FD_WRITE);
	return send_gift_command ("attach", NULL);
}
...
static int gift_stop ()
{
	...
	interface_send (gift_conn, "quit", NULL);
...
static void toggle_sharing ()
{
	...
	send_gift_command ("share", "action=s", if_state, NULL);
