...
char *mime_type (char *file, char *ext)
{
	...
	ext_dup = string_lower (STRDUP (ext_local));

	/* check for an existant and known extension, otherwise default to
	 * text/plain */
	if (ext_dup && (mime_info = dataset_lookup (mime_types, ext_dup)))
		type = mime_info->type;
	else
		type = "text/plain";
...
static off_t mime_size (char *file)
{
	off_t size = 0;
...
FILE *mime_open (char *file, char *mode, char **mime, off_t *size)
{
	FILE *f;
