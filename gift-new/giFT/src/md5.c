...
static unsigned char *md5_hash_file (char *file, unsigned long size)
{
	md5_state_t   state;
	char         *hash;
	char         *buf;
	...
	buf = malloc (st_blksize);

	while ((buf_n = read (fd, buf, MIN (st_blksize, st_size))) > 0)
	{
...
static unsigned char *md5_digest (char *file, unsigned long size)
{
	unsigned char *hash;
...
static char *md5_string (char *file, unsigned long size)
{
	unsigned char *hash;
	char          *hash_ascii;

	if (!(hash = md5_digest (file, size)))
		return NULL;

	hash_ascii = malloc (HASH_LEN * 2 + 1);
	bin_to_hex (hash, hash_ascii, HASH_LEN);
