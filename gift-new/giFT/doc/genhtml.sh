...
if [ $1 ]; then
    ...
    echo "Usage: $0 [docname] (without .tex)"
    echo "You need 'latex', 'tth' and 'tidy' for this script to work."
...
cat $doc.html | sed "s/<tt>    /<tt>\&nbsp;\&nbsp;\&nbsp;\&nbsp;/g" | sed "s/ width=\"[^\"]*\"//g" | egrep . | tidy -config tidyrc | sed "s/&\(.\)uml;/\&quot;\1/g" | sed "s/<\/tt><\/td>/\&nbsp;<\/tt><\/td>/g" | sed "s/<h2>/<hr \/><h2>/g" | sed "s/<title><\/title>//" | sed "s/<font face=\"helvetica\"><b><font size=\"+4\"><\/font><\/b><\/font> /<b><font face=\"helvetica\" size=\"+3\">/" | sed "s/<font size=\"+4\">//" | sed "s/<font size=\"+0\">/<\/font>/" | sed "s/<\/font><\/font>/<\/b>/" | sed "s/<tt>\\$/<tt>\&nbsp;\&nbsp;\$/g" | tac | sed "1,/<\/div>/d" | tac >> $doc.tmp
if [ $doc == whatis ]; then
