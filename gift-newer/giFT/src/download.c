...
static int download_report (Transfer *transfer)
{
	if (transfer->chunks)
	{
		...
		/* report individual chunk transfers */
		for (cptr = transfer->chunks; cptr; cptr = list_next (cptr))
		{
			...
			url    = (chunk->source ? chunk->source->url : NULL);
			status = source_status (chunk->source);
...
static int download_length (char *user)
{
	List *ptr;
	List *cptr;
	int   length = 0;
...
static void download_register (Transfer *transfer, Connection *c)
{
	...
	transfer->timer = timer_add (1 * SECONDS, (TimerCallback) download_report,
	                             transfer);
...
Transfer *download_new (Connection *c, char *filename, char *hash,
                        unsigned long size)
{
	Transfer *transfer;

	transfer = transfer_new (TRANSFER_DOWNLOAD, filename, hash, size);
...
void download_stop (Transfer *transfer, int cancel)
{
	if (cancel && transfer->path)
	{
		...
		TRACE (("removing transfer files for %s", transfer->path));
		unlink (transfer->state_path);
...
static void activate_chunk (Chunk *chunk, Source *source)
{
	...
	{
		TRACE (("queueing %s (%s)", source->user, source->url));
...
static int deactivate_chunk (Chunk *chunk)
{
	...
	/* uncouple the chunk and source if this source completed.  it needs
	 * somewhere new to go
	 * NOTE: they remain coupled if it was unable to complete so that the
	 * timeout mechanism can catch it as if it were a timed out source */
	if (completed)
	{
	 ...
	 * forgot
	 * NOTE: this is a continuation of the logic above but must be handled
	 * after the protocol deals with itself for obvious reasons */
	if (completed)
		chunk->data = NULL;
...
static int open_output (Transfer *transfer)
{
	...
	if (!transfer->path)
	{
		...
		transfer->path       = STRDUP (incoming_path ("%s", uniq));
		transfer->state_path = STRDUP (incoming_path (".%s.state", uniq));
	...
	{
		TRACE (("failed to open %s for writing", transfer->path));
		perror ("fopen");
		return FALSE;
	}
...
static Transfer *download_read_state (char *file)
{
	...
	transfer = download_new (NULL, filename, hash, total);
