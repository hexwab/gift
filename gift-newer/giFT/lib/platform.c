...
# define AF_LOCAL 1
# define R_OK 04
  /* NSIS defines this for us! */
# define GIFT_REG_KEY "Software\\giFT\\giFT"
...
#ifdef WIN32
/* list of active threads, keyed on thread ID */
static HashTable *active_threads = NULL;
...
static int unix_init (char *argv0)
{
	...
	else
	{
		...
		if (h)
		{
			...
			if (!access (ph, R_OK))
				...
				GIFT_FATAL (("Unable to locate a sane home path, piss off"));
		}
	}
...
static int win32_init (char *argv0)
{
	WSADATA wsa_data;
	char   *slash;
	char    current_dir[_MAX_PATH] = {0};
	char    startup_dir[_MAX_PATH] = {0};
	...
	{
		GIFT_ERROR (("Failed to load winsock version %d.%d: Error %d",
		            WS_MAJOR, WS_MINOR, ret));
		return FALSE;
	}

	/* determine the startup directory */
	if (!(slash = strrchr (argv0, '\\')))
		slash = strrchr (argv0, '/');
		/* first we'll try argv0 */

	if (slash)
	{
		strncpy (startup_dir, argv0, slash - argv0);
		startup_dir[slash - argv0] = '\0';
		if (access (startup_dir, R_OK))
			startup_dir[0] = '\0';
	}
	...
	/* is there a HOME variable defined? */
	...
	if ((reg_dir = read_reg_dir (HKEY_LOCAL_MACHINE, GIFT_REG_KEY,
		...
		plugin_dir = STRDUP (startup_dir);
	else if (current_dir)
		plugin_dir = STRDUP (current_dir);
	else
	{
		GIFT_ERROR (("Can't find giFT directory: %s. "
					"Please reinstall giFT.", platform_last_error ()));
		return FALSE;
	}
	...
	{
		GIFT_ERROR (("Can't find %s directory: %s. "
					"Please reinstall giFT.", data_dir,
					platform_last_error ()));
		return FALSE;
	}
	...
	if ((reg_dir = read_reg_dir (HKEY_CURRENT_USER, GIFT_REG_KEY,
		...
		home_dir = STRDUP (startup_dir);
	else if (current_dir)
		home_dir = STRDUP (current_dir);
	else
	{
		GIFT_ERROR (("Can't find home directory: %s. "
					"Please reinstall giFT.", platform_last_error ()));
		return FALSE;
	}
	...
	else
	{
		...
		{
			GIFT_ERROR (("Can't find gift.conf: %s. "
						"Please reinstall giFT.", platform_last_error ()));
			return FALSE;
		}
	}
...
int platform_child_proc (ChildFunc c_func, ParentFunc p_func, void *udata)
{
	...
	/* if a parent func was supplied it indicates that there needs to be
	 * some form of communication between child and parent */
	if (p_func)
	{
	...
	/* note: you will need to change this if you want to use sdata with the
	 * parent func as well */
	sdata->fd    = pfd[1];
	sdata->udata = udata;
	...
	if (cpid == 0)
	{
		...
		/* notify the parent that this child needs to be waited upon */
	...
	if (!active_threads)
		active_threads = hash_table_new ();

	/* so we can look up the thread handle by the thread identifier */
	/* why there is an ID *and* a handle is beyond me */
	hash_table_insert (active_threads, (unsigned long) cpid, hThread);
#endif /* !WIN32 */
...
static void child_exit (int signum)
{
	/* we don't want zombie child processes */
	waitpid (-1, NULL, WUNTRACED | WNOHANG);
}
#endif /* !WIN32 */

/*****************************************************************************/
/* PLATFORM_PATH_TO_HOST and PLATFORM_PATH_TO_GIFT */
...
char *platform_path_to_host (char *gift_path)
{
	...
	host_path = strdup (gift_path);

#ifdef WIN32
	/* /C -> C: */
	host_path[0] = gift_path[1];
	host_path[1] = ':';
...
char *platform_path_to_gift (char *host_path)
{
	...
	gift_path = strdup (host_path);

#ifdef WIN32
	/* C: -> /C */
	gift_path[0] = '/';
	gift_path[1] = host_path[0];

	for (ptr = gift_path; *ptr; ptr++)
		if (*ptr == '\\')
			*ptr = '/';
#endif
...
/* I don't know if we need a platform_perror () too */

void platform_net_perror (const char *msg)
{
	fprintf (stderr, "%s: %s\n", msg, platform_last_net_error());
}
