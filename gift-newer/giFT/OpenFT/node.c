                                    ...
                                    unsigned long *ip)
{
	...
	ft_packet_send (c, FT_STATS_REQUEST, "%hu%lu",
	                3 /* remove users shares */, ntohl (*ip));
                        ...
                        unsigned short gain, unsigned short loss)
{
	...
	else
	{
		...
		if (gain)
		{
			len += sprintf (fmt_change + len, "+%s%s",
			                node_class_str (gain), (loss ? " " : ""));
		}

		if (loss)
		{
			len += sprintf (fmt_change + len, "-%s",
			                node_class_str (loss));
		}

		sprintf (fmt_change + len, ")");
	}
