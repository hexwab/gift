                                        ...
                                        Connection *child)
{
	...
	/* submit the digest */
	ft_packet_send (c, FT_STATS_REQUEST, "%hu%lu%lu%lu",
	                2 /* submit digest */,
	                ntohl (NODE (child)->ip), shares, (unsigned long) size);
                                  ...
                                  unsigned long *id)
{
	...
	if (NODE (c)->ip != share->host_share->host)
	{
		/* use hpath when present (search_local will return results w/
		 * valid hpaths) */
		ft_packet_send (c, FT_SEARCH_RESPONSE, "%lu%lu%hu%hu%lu%lu%s%s",
						P_INT (id),
		                ntohl (share->host_share->host),
...
FT_HANDLER (push_request)
{
	 ...
	 * in a later function... */
	file_share = share_find_file (file);

	/* NOTE: range request is NOT handled here.  Once we establish the
	 * remote connection, the user will request the range it wishes to
	 * receive */
	xfer       = http_transfer_new (ft_upload, ip, port, file, 0, 0);
	xfer->hash = ((file_share && file_share->sdata) ?
	              STRDUP (file_share->sdata->md5) : NULL);

	transfer = upload_new (p, net_ip_str (ip), xfer->hash,
	                       file_basename (file), file, 0, 0, FALSE);
	 ...
	 case 2: /* SUBMIT USER SHARES REPORT */
		{
			...
			user   = ft_packet_get_int (packet, 4, FALSE); /* network order */
