...
static Dataset *response_parse (char *http_response)
{
	...
	/* response is something like HTTP/1.0 200 OK, we're trying to get 200 */
	/*  */ string_sep (&response, " ");
	code = string_sep (&response, " ");

	/* should have it now */
	dataset_insert (dataset, "code", STRDUP (code));
                              ...
                              int is_push, int nodepage)
{
	...
	if (verify_share)
	{
		/* nodepage should fall through as verified */
		if (!nodepage)
		{
...
int handle_response_code (Chunk *chunk, FT_Transfer *xfer)
{
	int code;
	...
	code = ATOI (dataset_lookup (xfer->header, "code"));

	if (!code)
	{
		TRACE (("no response code found..."));
		return FALSE;
	}

	/* hmm, i have no idea how http.c works anymore */
	...
	/* assert that valid sources are noted as such by giFT */
	if (code == 200 && chunk->source->status != SOURCE_ACTIVE)
	{
		TRACE (("status = %i???", chunk->source->status));
		chunk->source->status = SOURCE_ACTIVE;
	}

	/* handle error codes
	 * TODO -- 404 should be moved once 0.0.5 comes out */
	if (code == 404 || code == 503)
	{
		/* do not set this if we're already queueing locally */
		if (chunk->source->status != SOURCE_QUEUED_LOCAL)
...
static int read_response (Protocol *p, Connection *c)
{
	...
	/* handle 200, 404, ... and respond to giFT appropriately */
	if (!handle_response_code (chunk, xfer))
	{
		http_connection_close (c, chunk, xfer);
		return FALSE;
	}
...
static int incoming_connection (Protocol *p, Connection *c)
{
	...
	if (!strcasecmp (rtype, "PUSH"))
	{
		 ...
		 * to do this */
		xfer->range_start = chunk->start + chunk->transmit;
		 ...
		 * the file */
		send_reply_header (xfer, FALSE, xfer->range_start, xfer->range_stop,
		                   xfer->range_stop - xfer->range_start, TRUE, FALSE);
...
int http_push_file (Chunk *chunk)
{
	...
	c->fd = net_connect (net_ip_str (xfer->ip), xfer->port, FALSE);
