...
static List *spread_final = NULL;      /* connected    */
static List *spread_final_iter = NULL;
static List *spread_limbo = NULL;      /* connecting   */
static List *spread_limbo_iter = NULL;
static List *spread_disco = NULL;      /* disconnected */
...
#ifdef USE_CONN_SPREAD
static List **spread_op_list (unsigned short state, List ***r_op_iter)
{
	List **op_list = NULL;
	List **op_iter = NULL;
...
# endif /* CHECK_CONN_SPREAD */
	 ...
	 * beginning */
	ptr = (iter) ? *op_iter : *op_list;
	*op_list = list_lock (*op_list);
	...
	if (*op_iter)
	{
		for (ptr = (*op_list)->head->pending_remove; ptr; ptr = list_next (ptr))
		{
			if (ptr->data == (*op_iter)->data)
			{
				*op_iter = NULL;
				break;
			}
