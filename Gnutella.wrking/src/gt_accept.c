/*
 * $Id: gt_accept.c,v 1.61 2004/06/02 07:13:56 hipnod Exp $
 *
...
static void determine_method (int fd, input_id id, struct incoming_conn *conn)
{
	...
	if (len <= 0)
	{
		if (HANDSHAKE_DEBUG)
		{
			if (len < 0)
				...
				GT->DBGSOCK (GT, c, "recv: socket closed at remote end");
		}
	...
	/*
	 * Mark not firewalled if the other end isn't local as some data has been
	 * successfully read at this point.
	 */
	if (!c->outgoing && !net_match_host (net_peer (fd), "LOCAL"))
	{
		if (GT_SELF->firewalled)
		{
			GT->DBGFN (GT, "connection from %s, clearing firewalled status",
			           net_peer_ip (fd));
		}
	...
	/*
	 * Check if the request was not a complete line. We close the request
	 * if so to prevent polling the CPU, because we leave the data on the
	 * socket. This should be rare though, and is much easier to implement for
	 * now.
	 */
	if (!strchr (buf, '\n'))
	{
		if (HANDSHAKE_DEBUG)
		{
			GT->DBGSOCK (GT, c, "incomplete accept line (len %d), closing",
			             len);
		}
...
GT_METHOD(gt_server_accept)
{
	 ...
	 */
	gnutella_parse_response_headers (request, &GT_NODE(c)->hdr);
