/*
 * $Id: gt_web_cache.c,v 1.63 2004/03/24 06:58:09 hipnod Exp $
 *
...
{
	/*
	 * Don't use GWebCaches if this node is old.
	 */
	if (time (NULL) - GT_RELEASE_DATE >= EDAYS * 30 * 6)
	{
		static BOOL warned = FALSE;

		if (!warned)
		{
			GT->DBGFN (GT, "Not updating from webcaches: servent %lu days old",
			           (time (NULL) - GT_RELEASE_DATE) / EDAYS);
		}
