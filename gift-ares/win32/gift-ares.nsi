;------------------------------------------------------------------------------------------
; This file creates an installer for gift-ares for use with KCeasy, giFTwin32 or
; other installations of giFT on windows. It assumes gift-ares was build using the
; environment provided by giFT.
;
; This installer expects two Ares DLLs:
; - A Ares.dll compiled with RELEASE=1 for giFTwin32 and custom installs.
; - A Ares-Loader.dll compiled with RELEASE=1 and USE_LOADER=1 for KCeasy.
;------------------------------------------------------------------------------------------
...
Name "Ares plugin for giFT"
Caption "giFT-Ares ${VERSION} Setup"
OutFile ${OUTFILE}
...
Section "gift-ares for KCeasy" SEC_KCEASY
	SectionIn 1
...
FoundInstallPath:
	...
	File /oname=Ares.dll Ares-Loader.dll

	SetOutPath $INSTDIR\giFT\conf\Ares
	File ${TMP_DIR}\Ares.conf
	File ${DIST_DIR}\data\Ares\nodes

	; update giftd.conf
	ReadINIStr $R1 $INSTDIR\giFT\conf\giftd.conf "main" "plugins"
	IfErrors ConfError

	; add ":Ares" to plugin line if not present
	Push $R1
	Push "Ares"
	Call StrStr
	Pop $R0
	StrCmp $R0 "" 0 ConfError
	WriteINIStr $INSTDIR\giFT\conf\giftd.conf "main" "plugins" "$R1:Ares"
...
Section "gift-ares for giFTwin32" SEC_GIFTWIN32
	SectionIn 1
...
FoundInstallPath:
	StrCpy $INSTDIR $R1
	SetOutPath $INSTDIR
	File /oname=Ares.dll Ares.dll

	SetOutPath $INSTDIR\Ares
	File ${TMP_DIR}\Ares.conf
	File ${DIST_DIR}\data\Ares\nodes
	...
	Pop $R0
	StrCmp $R0 "" 0 ConfError
	WriteINIStr $INSTDIR\giftd.conf "main" "plugins" "$R1:Ares"
ConfError:
ExitSection:
SectionEnd


Section /o "gift-ares in custom directory" SEC_CUSTOM
	SectionIn 1
