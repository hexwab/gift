/*
 * meta_ogg.c -- inappropriately named, this is actually for parsing the
 * Ogg Vorbis codec
 *
...
int meta_ogg_run (FileShare *file, char *path)
{
	...
	if (bitrate > 0)
		meta_set (file, "bitrate", stringf ("%likbps", bitrate / 1000));

	meta_set (file, "frequency", stringf ("%liHz", freq));
	meta_set (file, "duration",  stringf ("%is", dur));
