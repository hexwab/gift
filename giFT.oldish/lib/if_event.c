...
/* TODO - opt */
static IFEventID uniq_id (Dataset *d, IFEventID *counter)
{
	IFEventID cnt;

	cnt = (counter ? *counter : 0) + 1;
                                       ...
                                       IFEventID session_id)
{
	...
	if (session_id == 0)
		session_id = uniq_id (ifc->events, NULL);
	else
	{
                       ...
                       char *data_name, void *data)
{
	...
	event->initiate      = c;
	event->id            = uniq_id (events, &id_counter);
	event->flags         = flags;
	...
	dataset_insert (&events, &event->id, sizeof (event->id), event, 0);
