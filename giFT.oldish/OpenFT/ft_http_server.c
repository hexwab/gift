...
static int http_server_send_error (Connection *c, int code)
{
...
static void server_handle_push (FTTransfer *xfer)
{
	    ...
	    (code = ATOI (code_str)) >= 300)
	{
		...
		queue_pos = dataset_lookupstr (xfer->header, "X-QueuePosition");

		openft_p->source_status (openft_p, chunk->source, SOURCE_QUEUED_REMOTE,
...
static Transfer *start_upload (FTTransfer *xfer)
{
	...
	transfer = upload_new (openft_p, user, xfer->hash,
...
void server_upload_file (Protocol *p, Connection *c)
{
	...
	if (!(read_len = fread (buf, sizeof (char), sizeof (buf), xfer->f)))
	{
