                         ...
                         char *query, char *exclude, char *realm)
{
	...
	memset (search, 0, sizeof (FTSearch));
                         ...
                         FileShare *file, FTShare *share)
{
	...
	user = STRDUP (node_user (share->shost->host, share->shost->alias));
	node = STRDUP (node_user (FT_NODE(snode)->ip, NULL));
...
static Connection *send_search (Connection *c, Node *node, FTSearch *search)
{
	...
	ft_search_need_reply (search, FT_NODE(c)->ip);
	return NULL;
}
                        ...
                        char *query, char *exclude, char *realm, Dataset *meta)
{
	 ...
	 * right now so we will return to gift that we dont need any more time
