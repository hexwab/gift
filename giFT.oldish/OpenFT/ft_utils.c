...
static void accept_test_result (Connection *c, Connection *verify, int success)
{
	...
	if (!success && FT_NODE(c)->port != 0)
	{
		...
		node_conn_set (c, 0, 0, -1, NULL);
	}

	if (verify)
	{
		if (verify == FT_NODE(c)->port_verify)
			FT_NODE(c)->port_verify = NULL;
		else
		if (verify == FT_NODE(c)->http_port_verify)
			FT_NODE(c)->http_port_verify = NULL;
	...
	if (FT_NODE(c)->port_verify || FT_NODE(c)->http_port_verify)
		return;
...
void ft_accept_test (Connection *c)
{
	 ...
	 * occur */
	accept_test_port (c, FT_NODE(c)->port,      &(FT_NODE(c)->port_verify));
	accept_test_port (c, FT_NODE(c)->http_port, &(FT_NODE(c)->http_port_verify));
