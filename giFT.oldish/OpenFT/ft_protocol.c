...
FT_HANDLER (nodeinfo_request)
{
	...
	node_c = (ip ? conn_lookup (ip) : OPENFT->ft);

	ft_packet_sendva (c, FT_NODEINFO_RESPONSE, 0, "Ihhhs",
	                  FT_NODE(node_c)->ip, FT_NODE(node_c)->class,
	                  FT_NODE(node_c)->port, FT_NODE(node_c)->http_port,
	                  FT_NODE(node_c)->alias);
}
...
FT_HANDLER (nodeinfo_response)
{
	...
	if (FT_NODE(c)->class & NODE_CHILD)
		klass |= NODE_CHILD;

	if (klass & NODE_SEARCH && FT_NODE(c)->class & NODE_PARENT)
		klass |= NODE_PARENT;

	node_class_set (c, klass);
	node_conn_set (c, 0, port, http_port, alias);

	/* we need to verify that this node actually accepts connections on the
	 * configured ports (damn liars!) */
	if (!FT_NODE(c)->incoming)
	{
		FT_NODE(c)->verified = TRUE;
                                 ...
                                 void *udata)
{
	...
	if (!response)
	{
		FT_NODE(c)->sent_list = TRUE;
		ft_packet_sendva (c, FT_NODELIST_RESPONSE, 0, NULL);
		return FALSE;
	}

	/* be curteous ;) */
	if (FT_NODE(c)->ip == response->ip)
		return FALSE;
...
FT_HANDLER (nodelist_request)
{
	...
	nodes = conn_length (NODE_SEARCH | NODE_INDEX, NODE_CONNECTED);

	conn_foreach ((ConnForeachFunc) nodelist_output, &out_list,
				  NODE_SEARCH | NODE_INDEX, NODE_CONNECTED, 15);

	if (nodes < 10)
	{
		conn_foreach ((ConnForeachFunc) nodelist_output, &out_list,
		              NODE_USER, NODE_CONNECTED, 10);
	}
...
FT_HANDLER (nodecap_request)
{
	...
	dataset_foreach (FT_SELF->cap, DATASET_FOREACH (add_nodecap), pkt);
...
FT_HANDLER (nodecap_response)
{
	...
	while ((key_id = ft_packet_get_uint16 (packet, TRUE)))
	{
		...
		dataset_insertstr (&FT_NODE(c)->cap, key, key);
	}
}
...
static int is_child (Connection *c)
{
	...
	return (FT_NODE(c)->class & NODE_CHILD);
}
...
FT_HANDLER (addshare_request)
{
	...
	ft_share_add (FT_NODE(c)->verified, FT_NODE(c)->ip, FT_NODE(c)->port,
...
static Connection *submit_digest_index (Connection *c, Node *node,
                                        Connection *child)
{
	...
	/* submit the digest */
	ft_packet_sendva (c, FT_STATS_REQUEST, 0, "hIll", 2 /* SUBMIT DIGEST */,
...
{
	conn_foreach ((ConnForeachFunc) submit_digest_index, c,
...
FT_HANDLER (stats_response)
{
	...
	FT_NODE(c)->stats.users  = users;
	FT_NODE(c)->stats.shares = shares;
	FT_NODE(c)->stats.size   = size;
}
