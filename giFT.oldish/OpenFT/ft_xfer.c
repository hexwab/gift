                              ...
                              off_t start, off_t stop)
{
	...
	memset (xfer, 0, sizeof (FTTransfer));
...
void ft_transfer_status (FTTransfer *xfer, SourceStatus status, char *text)
{
	...
	ft_transfer_unref (&c, &chunk, &xfer);
	openft_p->source_status (openft_p, chunk->source, status, text);
}

/*****************************************************************************/

#ifdef PERSISTENT_HTTP
static int find_open (Connection *c, in_addr_t *ip)
{
	char *c_str;
	char *ip_str;
	int   ret;
...
static Connection *lookup_connection (in_addr_t ip, unsigned short port)
{
	List       *link;
	Connection *c = NULL;
...
#endif /* PERSISTENT_HTTP */
		c = connection_open (openft_p, net_ip_str (ip), port, FALSE);
...
FILE *ft_transfer_open_request (FTTransfer *xfer, int *code)
{
	...
	if (auth)
	{
		if (!file_exists (host_path, &xfer->open_path_size, NULL))
		{
			free (host_path);
			return NULL;
		}
...
{
	openft_p->chunk_write (openft_p, chunk->transfer, chunk, chunk->source,
	                       segment, len);
}
						   ...
						   Source *source)
{
	...
	{
		openft_p->source_status (openft_p, chunk->source,
		...
		if (search_ip == 0)
		{
			...
			ft_packet_sendva (conn_lookup (ip), FT_PUSH_REQUEST, 0, "Ihsll",
			                  0, 0, request, (ft_uint32) start, (ft_uint32) stop);
		}
		else
		{
			 ...
			 * from */
			ft_packet_sendva (conn_lookup (search_ip), FT_PUSH_RESPONSE, 0,
			                  "Ihsll", ip, port, request,
			                  (ft_uint32) start,
			                  (ft_uint32) stop);
		}
...
static Connection *submit_uploads (Connection *c, Node *node,
								   unsigned long *avail)
{
	ft_packet_sendva (c, FT_MODSHARE_REQUEST, 0, "hl", FALSE, *avail);
	return NULL;
}
...
{
	conn_foreach ((ConnForeachFunc) submit_uploads, &avail,
