...
#define PRIVATE_NODEPAGE config_get_int (openft_conf, "nodepage/private=0")
#define PRIVATE_ALLOW    config_get_str (openft_conf, "local/hosts_allow=LOCAL")
...
static void build_nodelist (Connection *c, Node *node, FILE *f)
{
	...
	{
		ft_share_host_digest (node->ip, &shares, &size,
							  &uploads, &max_uploads);
...
char *html_page_index (unsigned long ip, char *file, char *sec)
{
	FILE *f;
