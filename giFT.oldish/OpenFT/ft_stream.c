...
static ft_uint32 stream_id (Connection *c)
{
	...
	while (dataset_lookup (FT_NODE(c)->streams_recv, &id_cnt, sizeof (id_cnt)) ||
	       dataset_lookup (FT_NODE(c)->streams_send, &id_cnt, sizeof (id_cnt)))
	{
		id_cnt++;
	}
                             ...
                             ft_uint16 cmd)
{
	...
	memset (stream, 0, sizeof (FTStream));
...
static Dataset **get_direction (Connection *c, FTStreamDirection dir)
{
	...
	{
	 case FT_STREAM_RECV:
		d = &(FT_NODE(c)->streams_recv);
		break;
	 case FT_STREAM_SEND:
		d = &(FT_NODE(c)->streams_send);
		break;
	}
...
#ifdef USE_ZLIB
		if (dataset_lookup (FT_NODE(c)->cap, "ZLIB", 5))
		{
