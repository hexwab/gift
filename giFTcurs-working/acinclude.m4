...
dnl 
dnl /* If your Curses does not have color define this one */
...
if test "x$slrn_cv_va_copy" = "xyes"; then
  ...
  if test "x$slrn_cv___va_copy" = "xyes"; then
	...
	if test "x$slrn_cv_va_val_copy" = "xyes"; then
	  AC_WARN("Faking va_copy is unsafe and may fail. Upgrade your compiler.")
	else
	  AC_ERROR("No working va_copy variant found. Upgrade your compiler.")
	fi
  fi
fi
])
