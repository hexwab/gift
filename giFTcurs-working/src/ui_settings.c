 ...
 *
 * $Id: ui_settings.c,v 1.70 2002/11/08 17:54:07 chnix Exp $
 */
#include "giftcurs.h"
...
static int settings_screen_handler(int key)
{
	...
	case FIELD_RESET_BUTTON:
		if (PRESSED(key)) {
			...
			ui_draw();
			refresh();
			message(_("Default color theme loaded."));
		}
		break;
	case FIELD_SYNC:
		if (PRESSED(key))
...
ui_methods settings_screen_methods = {
