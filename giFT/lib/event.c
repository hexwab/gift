/*
 * $Id: event.c,v 1.92 2007/05/16 20:40:21 hexwab Exp $
 *
...
#define              MAX_INPUTS      16384   /* maximum number of inputs */
...
static void dispatch_input (Input *input)
{
	...
	{
		GIFT_TRACE (("Input: fd = %d, id = 0x%X, validate = 0, complete = FALSE. "
		             "Input suspension problem?", input->fd, input->id));
	}
...
#else
		if (ret < 0 && platform_net_errno() == WSAENOTSOCK)
#endif
		{
...
#else
			if (platform_net_errno() == WSAENOTSOCK)
#endif
				bad_fd_abort (poll_fds, poll_ids);

			/* Show error if it's not an interrupted syscall */
#ifndef WIN32
			if (platform_net_errno() != EINTR)
#else
			if (platform_net_errno() != WSAEINTR)
#endif
				GIFT_ERROR ((POLLFN ": %s", GIFT_NETERROR()));

#ifdef WIN32
			/* 
			 * This seems to happen on Windows for some people. I am not sure
			 * if it is some bug or a genuine lack of kernel resources. For
			 * now just sleep the amount of time the select call would have
			 * lasted so we do not fill up the log and hog the CPU.
			 */
			if (platform_net_errno() == WSAENOBUFS)
			{
				MEMORYSTATUS memstat;
				memstat.dwLength = sizeof(memstat);
				GlobalMemoryStatus(&memstat);

				GIFT_TRACE (("nfds: %d, mem load: %d%%, physical mem: %d KB/%d KB",
				             poll_ids, memstat.dwMemoryLoad,
				             memstat.dwAvailPhys / 1024,
				             memstat.dwTotalPhys / 1024));

				Sleep ((timeout < 3 * SECONDS) ? timeout : (3 * SECONDS));
			}
#endif
		}
		break;
