/*
 * $Id: share_cache.c,v 1.76 2006/11/19 18:23:56 hexwab Exp $
 *
...
#define HIDDEN_EXTS \
	config_get_str (gift_conf, "sharing/hidden_extensions=")
...
static void consolidate_share_root (List **sroot)
{
	List *l, *w, *new_sroot = NULL;

	/* remove all paths which are subpaths of other paths. */
	for (l = *sroot; l; l = l->next)
	{
		struct _share_root *sl = l->data;

		if (!sl)
			continue;

		/* see if there is a path including this one */
		for (w = *sroot; w; w = w->next)
		{
			struct _share_root *sw = w->data;
			char *lpath;
			int wlen, llen;
			...
			lpath = STRDUP (sl->path);
			if (llen > wlen && lpath[wlen] == '/')
				lpath[wlen] = 0;
			
#ifndef WIN32
			if(!strcmp (lpath, sw->path) &&
#else
			if(!strcasecmp (lpath, sw->path) &&
#endif
			   (llen == wlen || (sw->recurse && llen > wlen)))
			{
				/* this path is subpath of another one. remove it. */
				GIFT_WARN (("Ignoring share root '%s' because it is already "
				            "part of '%s'", sl->path, sw->path));
				free_share_root (sl, NULL);
				l->data = NULL;
				free (lpath);
				break;
			}
	...
	/* now copy entries not NULLified above */
	for (l = *sroot; l; l = l->next)
		if (l->data)
			new_sroot = list_prepend (new_sroot, l->data);

	/* free old list */
	*sroot = list_free (*sroot);
	*sroot = new_sroot;
}
...
#if 0
	GIFT_TRACE (("Share root before consolidation:"));
	for (l = sroot; l; l = l->next)
		GIFT_TRACE (("root: %s", ((struct _share_root *)l->data)->path));
#endif
	
	/* Prevent directories from being added twice. Specifically prevent
	 * completed dir from being added through sharing root as well. */
	consolidate_share_root (&sroot);

#if 0
	GIFT_TRACE (("Share root after consolidation:"));
	for (l = sroot; l; l = l->next)
		GIFT_TRACE (("root: %s", ((struct _share_root *)l->data)->path));
#endif
...
static char *calculate_root (List *sroot, char *filename)
{
	...
	for (ptr = sroot; ptr; ptr = list_next (ptr))
	{
		...
		if (flen > plen && fname[plen] == '/')
			fname[plen] = 0;

		/* Our path handling makes the baby jesus cry :( */
#ifndef WIN32
		if(!strcmp (fname, sr->path))
#else
		if(!strcasecmp (fname, sr->path))
#endif
		{
			free (fname);
			return sr->path;
                          ...
                          int recurse)
{
	    ...
	    (incoming_dir = STRDUP (INCOMING_PATH (("")))))
	{
		...
		int incoming_len = strlen (incoming_dir);
		...
		if (strlen (current_dir) > incoming_len &&
		    ((current_dir[incoming_len] == '/' ||
			  current_dir[incoming_len] == '\\' )))
		{
			current_dir[incoming_len] = 0;
		}
                              ...
                              unsigned long *r_files, double *r_size)
{
	...
	GIFT_TRACE (("Entered share_build_index()"));

	if (!(db = sdb_open (gift_conf_path ("shares"), "rb")))
		GIFT_TRACE (("*** creating new shares file"));
	else
	{
		/* sdb_read will return NULL either if we have read all records or
		 * if a corrupted record was detected in which case we cannot safely
		 * read further records anyway.
		 */
...
void share_write_index (SubprocessData *subproc)
{
	...
	GIFT_TRACE (("Entered share_write_index()"));
...
Dataset *share_read_index (void)
{
	...
	GIFT_TRACE (("Entered share_read_index()"));
