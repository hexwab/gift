/*
 * $Id: daemon.c,v 1.101 2005/04/22 15:34:14 mkern Exp $
 *
...
static void handle_search (TCPC *c, Interface *cmd, IFSearchType type)
{
	...
	if (!(event = if_search_new (c, requested, type, query, exclude,
	                             realm, NULL)))
	{
		/* This leaves the FE hanging forever since we did not get to sending
		 * in an empty ITEM. Oh well.
		 */
		return;
	}

	/*
	 * If one or more plugin names were provided, send the search request only
	 * to those plugins. Otherwise broadcast to all.
	 */
	if (STRING_NULL(pname))
	{
		char *proto;
		char *pname0 = pname = STRDUP (pname);

		/* pname may be multiple protocol names separated by spaces. */
		while ((proto = string_sep_set (&pname, " ")))
		{
			/* Make sure plugin is actullaly loaded. */
			if (!(p = plugin_lookup (proto)))
			{
	...
	/* If we couldn't get at least one protocol to start searching finish the
	 * search thus sending the expected empty ITEM to the FE.
	 */
...
DCOMMAND (addsource)
{
	...
	if (!(transfer = lookup_download (user, hash, url, size)))
	{
		...
		if (transfer == NULL)
		{
			GIFT_ERROR (("new download failed (%s) (%d)", save, (int)size));
			return;
		}
	}
...
DCOMMAND (share)
{
	...
	GIFT_TRACE (("Front end sent SHARE(%s)", action));
...
DCOMMAND (quit)
{
	GIFT_TRACE (("Received QUIT from front end at %s", net_ip_str (c->host)));
	event_quit (0);
}
