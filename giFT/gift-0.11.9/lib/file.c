/*
 * $Id: file.c,v 1.49 2006/01/30 18:23:44 mkern Exp $
 *
...
BOOL file_truncate (FILE *f, off_t len)
{
	...
	/* Use fseek to flush all data to disk. We cannot use fflush since its
	 * behaviour is undefined after reads.
	 */
	off = ftell (f);
	fseek (f, MIN (off,len), SEEK_SET);
	
#ifdef _MSC_VER
	fd = _fileno (f); /* Never signals failure. */
