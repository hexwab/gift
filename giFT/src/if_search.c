/*
 * $Id: if_search.c,v 1.20 2005/04/16 19:43:43 mkern Exp $
 *
...
static void search_finished (TCPC *c, IFEvent *event, void *udata)
{
	 ...
	 *
	 * AFAICT search->protocols is empty on orderly (non cancelled) search
	 * completion since each protocol is removed in if_search_remove. Hence
	 * send_cancel is only called for the remaining plugins when the user
	 * cancels a search.
	 */
...
void if_search_remove (IFEvent *event, Protocol *p)
{
	...
	/* We no longer finish the search here if this was the last plugin. This 
	 * is now done in plugin.c since we need to be able to remove failed
	 * searches without finishing the search in daemon.c
	 */
	search->protocols = list_remove (search->protocols, p);
}
