/*
 * $Id: ft_sharing.c,v 1.15 2003/12/23 18:32:45 jasta Exp $
 *
...
static BOOL auth_child (FTNode *node)
{
	...
	/*
	 * We determine if we are able to support new children during handshaking
	 * and if not, we don't bother with any port verification.  As a result,
	 * we set child_eligibility to FALSE so that we know we can't allow this
	 * user to be our child should an open spot arise during this session.
	 */
	if (node->session->child_eligibility == FALSE)
	{
		FT->DBGSOCK (FT, FT_CONN(node), "TODO: Oops, skipped verification");
		return FALSE;
	}
