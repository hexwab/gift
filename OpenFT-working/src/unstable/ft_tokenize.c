...
static BOOL order_add (struct token_list *tlist, uint8_t num)
{
	if (num <= ORDER_SEP)
	{
		 ...
		 * sentinel */
#if 0
		printf ("ordtmp = %d, ordlen = %d\n", tlist->ordtmp, tlist->ordlen);
#endif
...
static void add_words (struct token_list *tlist, const char *str, int sepchar)
{
	size_t wordlen;
...
static void tlist_addstr (struct token_list *tlist, const char *str, int sepchar)
{
...
static uint32_t *tlist_finish (struct token_list *tlist, uint8_t **order)
{
	...
	/* add the sentinel (token=0), but only there are some
	 * tokens */
#if 0
	if (tlist->nmemb)
