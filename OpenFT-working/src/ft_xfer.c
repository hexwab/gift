/*
 * $Id: ft_xfer.c,v 1.45 2003/07/02 10:24:14 jasta Exp $
 *
...
void ft_transfer_log (FTTransfer *xfer)
{
	...
	if (chunk && transfer_direction (chunk->transfer) != TRANSFER_UPLOAD)
		return;
...
static void ft_transfer_free (FTTransfer *xfer)
{
	...
	/* uploads use these */
	if (xfer->share_authd && xfer->share_authd_alloc)
...
static Share *get_share_auth_openft (FTTransfer *xfer, char *hpath, int *code)
{
	Share         *share;
	char          *path_host;
	unsigned char *md5;
	...
	if (!(share = share_new (hpath)))
		return NULL;

	if (!(path_host = file_host_path (hpath)))
	{
		share_free (share);
		return NULL;
	}

	if (!(file_stat (path_host, &st)))
	{
		FT->err (FT, "cannot open %s for sharing", path_host);
		share_free (share);
		return NULL;
	}

	if ((md5 = md5_digest (path_host, 0)))
	{
		share_set_hash (share, "MD5", md5, 16, TRUE);
		free (md5);
	}

	share->mime = mime_type (path_host);
	...
	*code = 200;
	...
	/* do we need to free xfer->share_authd? */
	xfer->share_authd_alloc = TRUE;

	return share;
}

static Share *get_share_auth_gift (FTTransfer *xfer, char *hpath, int *code)
{
	Share         *share;
	int            ret;
	upload_auth_t  auth_nfo;
	 ...
	 */
	if ((share = FT->share_lookup (FT, SHARE_LOOKUP_HPATH, hpath)))
		ret = FT->upload_auth (FT, net_ip_str (xfer->ip), share, &auth_nfo);
	else
	...
	switch (ret)
	{
	 ...
	 case UPLOAD_AUTH_MAX_PERUSER:  *code = 503;  break;
	...
	{
		xfer->queue_pos = auth_nfo.queue_pos;
		xfer->queue_ttl = auth_nfo.queue_ttl;
	}

	xfer->share_authd_alloc = FALSE;
...
FILE *ft_transfer_open_request (FTTransfer *xfer, int *code)
{
	...
	assert (xfer != NULL);
	assert (xfer->request != NULL);
	assert (code != NULL);

	/* default value if we cant set something more meaningful */
	*code = 404;

	/* find the secured localized path based on the HTTP request */
	if (!(shared_path = localize_request (xfer, &preauth)))
		return NULL;

	/* if preauth is set it indicates that the share is authorized for upload
	 * by OpenFT as a special file and not coming from the internal share
	 * cache */
	if (preauth)
		share = get_share_auth_openft (xfer, shared_path, code);
	else
		share = get_share_auth_gift (xfer, shared_path, code);

	/* invalid request, likely because our upload slots are full, bail... */
	if (!share)
		return NULL;

	/* this is used by p->upload_start (p, ...) later */
	xfer->share_authd = share;

	if (!(host_path = file_host_path (share->path)))
		return NULL;

	/* stale entry that gift didnt tell us about? */
	if (!(f = fopen (host_path, "rb")))
	{
		FT->err (FT, "unexpected stale entry: %s", host_path);
		*code = 500;
	}
                           ...
                           Source *source)
{
	...
	if (!(xfer = ft_transfer_new (ip, port, start, stop)))
	{
		free (url);
		return FALSE;
	}
...
static BOOL submit_uploads (FTNode *node, unsigned long *avail)
{
	ft_packet_sendva (FT_CONN(node), FT_CHILD_PROP, 0, "l", (uint32_t)(*avail));
	return TRUE;
}
...
BOOL openft_source_add (Protocol *p, Transfer *transfer, Source *source)
{
	/* ...it seems we have nothing to do here */
	return TRUE;
}

BOOL openft_source_remove (Protocol *p, Transfer *transfer, Source *source)
{
	uint32_t      ip;
	char         *request;
	char         *url;
