/*
 * $Id: ft_search_db.c,v 1.61 2003/05/29 09:37:41 jasta Exp $
 *
...
static int compare_md5 (DB *dbp, const DBT *a, const DBT *b)
{
	 ...
	 * succeed */
	return INTCMP (a_rec->index, b_rec->index);
}
#endif
                             ...
                             FileShare *file)
{
	...
	share_foreach_meta (file, DS_FOREACH(pri_data_meta), &rec);
                          ...
                          FileShare *file)
{
	...
	key.data  = hash->data;
	key.size  = hash->len;
	data.data = &shost->index;
	data.size = sizeof (shost->index);
...
//		assert (data.size == sizeof (shost));
		assert (data.size == sizeof (index));
		//		shost = *(FHSHost **)data.data;
		memcpy (&index, data.data, sizeof (index));

		shost = ft_shost_get_index (index);
...
static int lookup_ret (DBT *dbt, void **args)
{
	...
	shost = ft_shost_get_index (rec->index);
	 ...
	 */
	if (shost)
		if (add_search_result (a, shost, rec->md5))
			(*matches)++;
...
BOOL ft_search_db_insert (FTSHost *shost, FileShare *file)
{
	BOOL ret = FALSE;
...
BOOL ft_search_db_remove (FTSHost *shost, unsigned char *md5)
{
	BOOL ret    = FALSE;
#ifdef USE_LIBDB
	off_t size = 0;
...
BOOL ft_search_db_sync (FTSHost *shost)
{
	BOOL ret = TRUE;
