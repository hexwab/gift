...
FTNode *ft_node_new (in_addr_t ip)
{
	...
	node->klass = NODE_USER;

	return node;
}

static int remove_packet (FTPacket *packet, void *udata)
{
...
void ft_node_free (FTNode *node)
{
	...
	if (node->session)
	{
		ft_session_stop (node->session->c);
		assert (node->session == NULL);
	}

	free (node->alias);

	list_foreach_remove (node->session_queue,
	                     (ListForeachFunc)remove_packet, NULL);
...
FTNode *ft_node_register_full (in_addr_t ip,
                               ...
                               FTNodeClass klass,
                               time_t vitality, time_t uptime, ft_uint32 ver)
{
	FTNode *node;
...
void ft_node_set_port (FTNode *node, unsigned short port)
{
	 ...
	 * class */
	if (!port && (node->klass & (NODE_SEARCH | NODE_INDEX)))
	{
		ft_node_remove_class (node, NODE_SEARCH);
		ft_node_remove_class (node, NODE_INDEX);
	}
}
...
void ft_node_queue (FTNode *node, FTPacket *packet)
{
	...
	node->session_queue = list_append (node->session_queue, packet);
}
                              ...
                              FTNodeClass klass)
{
	...
	TRACE (("%-24s %s %s", hostfmt, ft_node_classstr (klass), changefmt->str));
