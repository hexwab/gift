...
static struct conn_list final;         /* NODE_CONNECTED */
static struct conn_list limbo;         /* NODE_CONNECTING */
static struct conn_list disco;         /* NODE_DISCONNECTED */
...
static struct conn_list *get_conn_list (FTNodeState state)
{
	...
	switch (state)
	{
	 case NODE_CONNECTED:    list_addr = &final; break;
	 case NODE_CONNECTING:   list_addr = &limbo; break;
	 case NODE_DISCONNECTED: list_addr = &disco; break;
	}
...
static int add_sorted (FTNode *a, FTNode *b)
{
	...
	if ((ret = INTCMP(a->version, b->version)))
		return -ret;
                       ...
                       FTNetorgForeach func, void *udata)
{
	...
	else
	{
		...
		looped  = foreach_list (klass, NODE_CONNECTED, iter, func, udata);
		looped += foreach_list (klass, NODE_CONNECTING, iter, func, udata);
		looped += foreach_list (klass, NODE_DISCONNECTED, iter, func, udata);
	}
...
static int count_len (FTNode *node, int *len)
{
	(*len)++;
	return TRUE;
}
...
int ft_netorg_length (FTNodeClass klass, FTNodeState state)
{
	...
	ft_netorg_foreach (klass, state, 0,
	                   FT_NETORG_FOREACH(count_len), &len);
...
{
	TRACE (("%s: %hu", ft_node_fmt (node), node->klass));
	return TRUE;
}
