 ...
 * an obscure system or trying to debug a child process more easily.
 */
/* #define NO_CHILD */
...
static int child_new (SubprocessData *pdata, int *pfd)
{
	...
	if (!(cdata = memory_dup (pdata, sizeof (SubprocessData))))
		return FALSE;
...
int platform_child (ChildFunc c_func, ParentFunc p_func, void *udata)
{
	...
	memset (pdata, 0, sizeof (SubprocessData));
