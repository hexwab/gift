...
void log_trace (char *fmt, ...)
{
	...
	/* format this way so that we do not need to add whitespace between the
	 * file and the line but still maintain padding */
	snprintf (fileline, sizeof (fileline) - 1, "%s:%i",
	          trace_file, trace_line);

	buf_len += snprintf (buf + buf_len, sizeof (buf) - buf_len - 1,
	                     "%-21s %s", fileline, trace_func);
