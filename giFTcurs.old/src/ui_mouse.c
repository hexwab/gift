 ...
 *
 * $Id: ui_mouse.c,v 1.41 2003/05/16 13:30:45 chnix Exp $
 */
#include "giftcurs.h"
#include "screen.h"
...
# include <errno.h>
guint gpm_fd_tag = 0;
static gboolean gpm_mouse_event(GIOChannel * source, GIOCondition condition, gpointer data)
{
	MEVENT event;
	Gpm_Event gev;
...
void mouse_init(void)
{
	if (use_mouse) {
		...
		if (Gpm_Open(&conn, 0) < 0)
			...
			gpm_fd_tag = g_io_add_watch(g_io_channel_unix_new(gpm_fd), G_IO_IN,
										gpm_mouse_event, NULL);
			gpm_zerobased = 1;
		}
#endif
