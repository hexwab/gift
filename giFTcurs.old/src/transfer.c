 ...
 *
 * $Id: transfer.c,v 1.125 2003/05/14 09:09:35 chnix Exp $
 */
#include "giftcurs.h"
...
static char **download_sort_methods;
static char **upload_sort_methods;
static const char default_sort_methods[] = "filesize;transferred;bandwidth;filename";
static int upload_sort_method, download_sort_method;
static guint bandwidth_timer_tag = 0;

static void transfer_change_handle(ntree * packet, transfer *);
static void transfer_add_handle(ntree * packet, tree *);
...
void transfers_init(void)
{
	...
	if (!methods)
		methods = get_config("sort", "transfers", (char *) default_sort_methods);
	download_sort_methods = g_strsplit(methods, ";", -1);

	methods = get_config("sort", "uploads", NULL);
	if (!methods)
		methods = get_config("sort", "transfers", (char *) default_sort_methods);
	upload_sort_methods = g_strsplit(methods, ";", -1);

	upload_sort_method = download_sort_method = 0;
	...
	methods = upload_sort_methods[0];
	tree_sort(&uploads, methods);
	methods = download_sort_methods[0];
	tree_sort(&downloads, methods);
}
...
transfer *transfer_find(tree * tr, const char *hash, unsigned int size, int dead)
{
	int i;
...
static void transfer_add_handle(ntree * param, tree * transfer_tree)
{
	gift_id id;
	transfer *t;
...
static void transfer_change_handle(ntree * param, transfer * t)
{
	...
	if (!strcmp(command, "ADDSOURCE")) {
		...
		if (transfer_tree == &downloads)
			g_message(t->transferred == t->filesize ? _("Download of %s completed.") :
...
char *transfer_change_sort_method(tree * active_tree, int dir)
{
	...
	char ***sorting_methods =
		active_tree == &uploads ? &upload_sort_methods : &download_sort_methods;
	int *sort_index = active_tree == &uploads ? &upload_sort_method : &download_sort_method;
