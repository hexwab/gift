 ...
 *
 * $Id: list.c,v 1.76 2003/05/10 19:47:43 weinholt Exp $
 */
#include "giftcurs.h"
...
{
	g_assert(0 <= index && index < snafu->num);
	if (snafu->num == 1) {
		list_remove_all(snafu);
	} else {
...
{
	if (!nmemb) {
		g_free(snafu->entries);
		snafu->entries = NULL;
	} else if (!snafu->allocated) {
		snafu->entries = g_new(void *, nmemb);
	} else {
