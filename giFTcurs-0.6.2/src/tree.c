 ...
 *
 * $Id: tree.c,v 1.29 2003/07/22 23:58:37 weinholt Exp $
 */
#include "giftcurs.h"
...
void tree_release(leaf_ptr item, int idx)
{
	int changed = 1;
	tree_node *node = item;
	tree_node *parent = node->parent;
...
#ifndef NDEBUG
	g_assert(idx >= 0);
	g_assert(parent->children.entries[idx] == node);
	...
	if (parent->children.order == (CmpFunc) compare_sortkey)
		changed =
			make_sortkey(&node->sortkey, node, parent->klass->get_sort_order(parent),
						 node->klass->getattr);
	if (changed)
		changed = list_resort(&parent->children, idx);
...
void tree_resort(node_ptr item)
{
	...
	g_assert(t->klass);
	sortstring = t->klass->get_sort_order(t);
