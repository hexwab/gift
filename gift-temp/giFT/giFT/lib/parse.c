/*
 * $Id: parse.c,v 1.16 2003/05/05 11:53:47 jasta Exp $
 *
...
char *gift_strdup (const char *str)
{
	if (!str)
		return NULL;
...
char *gift_strncpy (char *dst, const char *src, size_t size)
{
	size_t len;
	...
	/* the length of the string we are copying is one smaller than the dest
	 * buffer allows, so that we can guarantee a \0 every time */
	len = size - 1;
