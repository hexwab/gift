/*
 * $Id: file.c,v 1.38 2003/04/25 11:48:13 jasta Exp $
 *
...
int file_stat (const char *file, struct stat *stbuf)
{
	if (!file || !stbuf)
		return FALSE;
...
int file_direxists (const char *path)
{
	struct stat st;
...
int file_mkdir (char *path, int mode)
{
	if (!path)
		return FALSE;
