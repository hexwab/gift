/*
 * $Id: dataset.c,v 1.18 2003/03/31 12:25:34 jasta Exp $
 *
...
static int d_list_lookup_by_key (DatasetNode *node, Array **args)
{
	void   *key;
	size_t *key_len;

	list (args, &key, &key_len, NULL);
...
static DatasetNode *d_list_lookup_node (Dataset *d, void *key, size_t key_len)
{
	...
	args = array (key, &key_len, NULL);
	link = list_find_custom (d->tdata.list, &args,
	                         (CompareFunc)d_list_lookup_by_key);
	unset (&args);

	return list_nth_data (link, 0);
}

static DatasetNode *d_array_lookup_node (Dataset *d, void *key, size_t key_len)
{
	Array       *a;
	DatasetNode *node;
	size_t       nmemb;
	...
	for (i = 0; i < nmemb; i++)
	{
		if (!(node = splice (&a, i, 0, NULL)))
			continue;

		if (dataset_cmp_node (node, key, key_len) == 0)
			return node;
	}
...
DatasetNode *dataset_lookup_node (Dataset *d, void *key, size_t key_len)
{
	...
	switch (d->type)
	{
	 case DATASET_LIST:
		...
		break;
	 case DATASET_ARRAY:
		node = d_array_lookup_node (d, key, key_len);
		break;
	 case DATASET_HASH:
...
static void d_array_insert (Dataset *d, DatasetNode *node)
{
	...
	node->tdata.array_offs = count (&(d->tdata.array));

	ret = push (&(d->tdata.array), node);
	assert (ret != NULL);
}
...
{
	splice (&(d->tdata.array), node->tdata.array_offs, 1, NULL);
}
...
static void d_array_foreach_remove (Dataset *d, DatasetForeach func, void *udata)
{
	Array       *a;
	DatasetNode *node;
	size_t       i;
...
void dataset_foreach (Dataset *d, DatasetForeach func, void *udata)
{
	...
	switch (d->type)
	{
	 case DATASET_LIST:
		...
		break;
	 case DATASET_ARRAY:
		d_array_foreach_remove (d, func, udata);
		break;
	 case DATASET_HASH:
...
static void d_array_foreach (Dataset *d, DatasetForeach func, void *udata)
{
	Array       *a;
	DatasetNode *node;
	size_t       i;
...
unsigned long dataset_length (Dataset *d)
{
	...
	{
	 case DATASET_LIST:
		cnt = list_length (d->tdata.list);
		break;
	 case DATASET_ARRAY:
		cnt = (unsigned long)count (&(d->tdata.array));
		break;
	 case DATASET_HASH:
		cnt = d->tdata.hash->items;
		break;
	 default:
		abort ();
		break;
