/*
 * $Id: md5.c,v 1.5 2003/05/05 09:49:12 jasta Exp $
 *
...
char *md5_string (unsigned char *md5, char *prefix)
{
	char    *md5_ascii;
	size_t   prefix_len = 0;
	...
	if (!(md5_ascii = malloc (HASH_LEN * 2 + prefix_len + 1)))
		return NULL;

	if (prefix)
		memcpy (md5_ascii, prefix, prefix_len);

	if (!bin_to_hex (md5, md5_ascii + prefix_len, HASH_LEN))
	{
		free (md5_ascii);
		return NULL;
	}
