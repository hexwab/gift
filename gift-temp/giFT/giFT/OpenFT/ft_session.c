/*
 * $Id: ft_session.c,v 1.63 2003/05/05 09:49:11 jasta Exp $
 *
...
static void handle_packet (TCPC *c, unsigned char *data, size_t len)
{
	...
	if (session_auth_packet (c, packet))
		ret = protocol_handle (FT, c, packet);
...
static void connect_complete (TCPC *c, int incoming)
{
	...
	{
		ft_node_err (FT_NODE(c), FT_ERROR_UNKNOWN,
		             stringf ("Socket error: %s", GIFT_NETERROR()));
		ft_session_stop (c);
		return;
	}
