/*
 * $Id: ft_search.c,v 1.30 2003/05/05 09:49:10 jasta Exp $
 *
...
static void send_result (IFEvent *event, FileShare *file, FTShare *share,
                         FTNode *parent)
{
	String *s;
	char   *pathenc;
	...
	FT->search_result (FT, event, user, node, s->str,
	                   share->shost->availability, file);
...
void ft_search_reply (FTSearch *srch, FileShare *file, FTNode *parent)
{
	FTShare        *share;
	...
	if (!ft_search_cmp (file, params->type & ~FT_SEARCH_HIDDEN, params->realm,
	...
	assert (srch->event != NULL);

	/* otherwise, this is a normal search reply, send it back to the
	 * interface protocol */
	send_result (srch->event, file, share, parent);
}
...
void ft_browse_reply (FTBrowse *browse, FileShare *file)
{
	...
	send_result (browse->event, file, NULL, NULL);
}
...
static int result_reply (FileShare *file, FTSearch *search)
{
	...
	if ((share = share_lookup_data (file, "OpenFT")))
	{
		...
		if (share->shost->host)
		{
			ft_search_reply (search, file, FT_SELF);
			ret = TRUE;
		}
	}
...
static int search_parents (FTSearch *search)
{
	...
	n = ft_netorg_foreach (NODE_PARENT, NODE_CONNECTED, FT_SEARCH_PARENTS,
