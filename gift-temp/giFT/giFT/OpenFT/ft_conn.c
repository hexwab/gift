/*
 * $Id: ft_conn.c,v 1.9 2003/05/05 09:49:08 jasta Exp $
 *
...
int ft_conn_need_peers (void)
{
	...
	n = ft_netorg_length (NODE_SEARCH, NODE_CONNECTED);
...
int ft_conn_need_index (void)
{
	...
	n = ft_netorg_length (NODE_INDEX, NODE_CONNECTED);
...
static int keep_alive (void)
{
	...
	n = ft_netorg_foreach (NODE_INDEX, NODE_CONNECTED, 0,
	...
	p = ft_netorg_foreach (NODE_PARENT, NODE_CONNECTED, FT_SEARCH_PARENTS,
	                       FT_NETORG_FOREACH(send_heartbeat), sent);
	n += p;

	if (p < FT_SEARCH_PEERS && FT_SELF->klass & NODE_SEARCH)
	{
		int iter = FT_SEARCH_PEERS - p;

		/* keep alive all peer connections used to satisfy our childrens
		 * searches (as well as our own) */
		n += ft_netorg_foreach (NODE_SEARCH, NODE_CONNECTED, iter,
		                        FT_NETORG_FOREACH(send_heartbeat), sent);
	}
...
int ft_conn_maintain (void *udata)
{
	...
	/* not sure right now, but looks like a comment block should be here */
