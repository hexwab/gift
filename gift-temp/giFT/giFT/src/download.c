/*
 * $Id: download.c,v 1.234 2003/05/04 06:55:49 jasta Exp $
 *
...
static unsigned long  download_credits             = 0;
...
static int download_register (TCPC *c, IFEventID requested,
                              Transfer *transfer)
{
...
Transfer *download_new (TCPC *c, IFEventID requested,
...
static int verify_integrity (Transfer *transfer)
{
	...
	algo = hashstr_algo (transfer->hash);
	assert (algo != NULL);
...
static void fill_chunks (Transfer *transfer, char *chunks)
{
	...
	if (!file_stat (transfer->path, &st))
	{
		GIFT_ERROR (("unable to stat %s: %s", transfer->path, GIFT_STRERROR()));
		return;
	}
	...
	{
		start = (off_t)(ATOUL (string_sep (&token, "-")));
		stop  = (off_t)(ATOUL (token));
		...
		if (!transfer->chunks && start > st.st_size)
			start = st.st_size;
...
static Transfer *download_read_state (char *file)
{
	...
	/* hashes now need a prefix identifying the algorithm used */
	if (hash && !strchr (hash, ':'))
	{
		char new_hash[128];

		/* prefix the hash with "MD5:" if no prefix is found */
		snprintf (new_hash, sizeof (new_hash) - 1, "MD5:%s", hash);
		config_set_str (conf, "transfer/hash", new_hash);
