/*
 * $Id: if_transfer.c,v 1.9 2003/05/02 10:54:45 jasta Exp $
 *
                           ...
                           int child)
{
	...
	put_source (cmd, command, "statusgrl", source_status (source, FALSE));
	put_source (cmd, command, "status", source_status (source, TRUE));
...
IFEvent *if_transfer_new (TCPC *c, IFEventID requested,
                          Transfer *transfer)
{
	IFEvent *event;
