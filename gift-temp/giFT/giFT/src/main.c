/*
 * $Id: main.c,v 1.184 2003/05/05 11:55:14 jasta Exp $
 *
...
#include "download.h"                  /* download_stop(), _recover_state() */
...
static int    cl_help    = FALSE;
static int    cl_version = FALSE;
static int    cl_daemon  = FALSE;
static int    cl_idxonly = FALSE;
static Array *cl_opt     = NULL;
static Array *cl_proto   = NULL;
static int    cl_quiet   = FALSE;
...
static Protocol *load_plugin (char *proto)
{
	...
	if (file_exists (proto))
	{
		...
		STRNCPY (namebuf, name, MIN ((sizeof (namebuf)), proto - name));
		name = namebuf;
	}
	else
	{
