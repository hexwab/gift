/*
 *  $Id: meta_avi.c,v 1.11 2003/04/08 01:24:45 rossta Exp $
 *
...
int meta_avi_run (FileShare *file, char *path)
{
	avi_t *avifile;
	char  *audio_format;

	/* open file */
	if (!(avifile = AVI_open_input_file (path, 1)))
	{
	...
	if (avifile->anum > 0)
	{
		...
		meta_set (file, "audioformat", audio_format);
		meta_set (file, "audiofrequency", stringf ("%li", audiotrack->a_rate));
		//meta_set (file, "bits per sample", stringf ("%li bits", audiotrack->a_bits));
		meta_set (file, "audiobitrate",
				  stringf ("%li", (long)(audiotrack->mp3rate * 1000)));
	}

	/* extract video information */
	meta_set (file, "videoformat", avifile->compressor);
	meta_set (file, "resolution",
	          stringf ("%lix%li", avifile->width, avifile->height));
	meta_set (file, "fps", stringf ("%4.3f", avifile->fps));
	meta_set (file, "duration",
	          stringf ("%li",
	                   (long)((float)avifile->video_frames /
	                          (float)avifile->fps)));
	meta_set (file, "bitrate",
	          stringf ("%li",
	                   (long)((float)file->size /
...
void AVI_info (avi_t *avifile)
{
	...
	//GIFT_DEBUG(("[avilib] V: %6.3f fps, codec=%s, frames=%ld, width=%d, height=%d\n",  fps, ((strlen(codec)==0)? "RGB": codec), frames, width, height));
	...
	for (j = 0; j < tracks; ++j)
	{
		...
		/*
		 * if(chan>0) {
		 * GIFT_DEBUG(("[avilib] A: %ld Hz, format=0x%02x, bits=%d, channels=%d, bitrate=%ld kbps,\n", rate, format, bits, chan, mp3rate));
		 * GIFT_DEBUG(("[avilib]    %ld chunks, %ld bytes\n", chunks, tot_bytes));
		 * } else
		 * GIFT_DEBUG(("[avilib] A: no audio track found\n"));*/
	}
...
int AVI_file_check (char *import_file)
{
	...
	{
		GIFT_DEBUG (("(%s) invalid input file \"%s\"\n", __FILE__,
					 import_file));
		return (1);
	}
