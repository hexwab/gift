/*
 * $Id: meta_mp3.c,v 1.25 2003/03/31 12:25:37 jasta Exp $
 *
 * ripped from Gnapster
 *
...
static int process_mp3_header (FileShare *file, int fd, uint32_t hdr)
{
	...
	meta_set (file, "bitrate",   stringf ("%li", (long)(bitrate * 1000)));
	meta_set (file, "frequency", stringf ("%hu", freq));
	meta_set (file, "duration",  stringf ("%i", (int)dur));
