...
static void queue_write_default (Connection *c, char *data, void *udata)
{
	unsigned long len;
	...
	len = (udata ? (unsigned long) udata : strlen (data));

	send (c->fd, data, len, 0);
}

static void queue_destroy_default (Connection *c, char *data, void *udata)
{
	free (data);
}
...
static void queue_write (Protocol *p, Connection *c)
{
	...
	(*queue->func) (c, data, queue->udata);
	(*queue->destroy_func) (c, data, queue->udata);
