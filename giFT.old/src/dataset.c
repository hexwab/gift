...
Dataset *dataset_new ()
{
	HashTable *dataset       =  hash_table_new ();
...
{
	return hash_table_lookup (dataset, (unsigned long) string);
}

void dataset_insert_real (Dataset *dataset, char *key, void *value)
{
	hash_table_insert (dataset, (unsigned long) STRDUP (key),
	                   value);
}

void dataset_remove (Dataset *dataset, char *string)
{
	hash_table_remove (dataset, (unsigned long) string);
}
