...
/* data helpers */
#define SET_ROW_DATA(clist,row,data) gtk_clist_set_row_data_full (GTK_CLIST (clist), row, data, (GtkDestroyNotify) obj_destroy_notify)
#define SET_NODE_DATA(ctree,node,data) gtk_ctree_node_set_row_data_full (GTK_CTREE (ctree), node, data, (GtkDestroyNotify) obj_destroy_notify)

#define SET_PARENT(widget,ftapp) gtk_object_set_data (GTK_OBJECT (widget), "ft_app", ftapp)

/* get text at sort column of the given GtkCListRow ... */
#define ROW_GET_TEXT(clist,row) GTK_CELL_TEXT(((GtkCListRow *)row)->cell[GTK_CLIST(clist)->sort_column])->text
