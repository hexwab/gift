/*
 * $Id: ft_html.c,v 1.24 2003/06/24 19:57:20 jasta Exp $
 *
...
static void build_nodelist (FTNode *node, FILE *f)
{
	...
	assert (node->state == FT_NODE_CONNECTED);
	...
	if (FT_SEARCH_DB(node))
	{
		...
		shares = FT_SEARCH_DB(node)->shares;
		size   = FT_SEARCH_DB(node)->size;
		avail  = node->session->avail;
...
{
	if ((node->klass & FT_NODE_INDEX) || (node->klass & FT_NODE_SEARCH))
		return FALSE;
...
static int build_shares (FileShare *file, FILE *f)
{
	...
	if (!(encoded = url_encode (share_get_hpath (file))))
		return TRUE;

	if ((bitrate = share_get_meta (file, "bitrate")))
	{
		unsigned long bitrate_n = ATOUL (bitrate);
		bitrate_fmt = stringf_dup ("%lukbps", (bitrate_n / 1000));
	}

	if ((dur = share_get_meta (file, "duration")))
	{
