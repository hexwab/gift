/*
 * $Id: ft_query.c,v 1.25 2003/06/24 19:57:20 jasta Exp $
 *
...
static void sreply_result (sreply_t *reply, Share *file, FTNode *node)
{
	...
	ft_packet_put_ip     (pkt, node->ip);
	ft_packet_put_uint16 (pkt, node->port, TRUE);
	ft_packet_put_uint16 (pkt, node->http_port, TRUE);
	ft_packet_put_str    (pkt, node->alias);

	/* add this users "availability", that is, how many open upload slots thye
	 * currently have */
	ft_packet_put_uint32 (pkt, node->session->avail, TRUE);
...
static int search_result_logic (Share *file, sreply_t *reply)
{
	FTShare *share;
	FTNode  *shost;
	...
	/* access the node of the user sharing this file */
	if (!(shost = share->node))
		return FALSE;

	/* first drop all results that we matched from the node who requested
	 * the search directly */
	if (shost->ip == FT_NODE(reply->c)->ip)
		return FALSE;
	 ...
	 * guarantee that */
	if (shost->ip == reply->params->srcorig)
		return FALSE;

	/* now actually send the result */
	sreply_result (reply, file, shost);
	return TRUE;
}

static int search_result (Share *file, sreply_t *reply)
{
	int ret;
