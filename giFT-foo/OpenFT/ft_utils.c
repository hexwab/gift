/*
 * $Id: ft_utils.c,v 1.18 2003/06/24 19:57:20 jasta Exp $
 *
                              ...
                              TCPC **verify)
{
	...
	if (((*verify) = tcp_open (FT_NODE(c)->ip, port, FALSE)))
		(*verify)->udata = c;
...
void ft_accept_test (TCPC *c)
{
	...
	{
		FT->DBGFN (FT, "%s:%hu: connection already verified",
	 ...
	 * occur */
	accept_test_port (c, FT_NODE(c)->port, &(FT_SESSION(c)->verify_ft));
