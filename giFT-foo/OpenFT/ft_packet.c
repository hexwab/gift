/*
 * $Id: ft_packet.c,v 1.41 2003/06/06 04:06:32 jasta Exp $
 *
...
{
	if (!ft_packet_put_uint16 (packet, (uint16_t)4, TRUE))
		return FALSE;
...
BOOL ft_packet_put_ustr (FTPacket *packet, unsigned char *str, int len)
{
	if (!str || len == 0)
...
uint32_t ft_packet_get_uint (FTPacket *packet, size_t size, int host_order)
{
	...
	if (!packet || size == 0 || size > sizeof (uint32_t))
		return 0;
