/*
 * $Id: ft_http_server.c,v 1.36 2003/06/26 09:24:21 jasta Exp $
 *
...
static void reply_to_client_request (FTTransfer *xfer)
{
	 ...
	 */
	if (share->size)
		entity_size = share->size;
	else
	{
		FT->warn (FT, "using fallback entity size (%lu - %lu)",
		          (unsigned long)(xfer->stop), (unsigned long)(xfer->start));
	...
	snprintf (range, sizeof (range) - 1, "bytes %lu-%lu/%lu",
	          (unsigned long)(xfer->start),
	          (unsigned long)(xfer->stop - 1),
	          (unsigned long)(entity_size));

	snprintf (length, sizeof (length) - 1, "%lu",
	          (unsigned long)(xfer->stop - xfer->start));

	hash_dsp = share_dsp_hash (share, "MD5");
...
static void get_client_request (int fd, input_id id, TCPC *c)
{
	 ...
	 * in on this
	 */
	xfer = ft_transfer_new (net_ip (net_peer_ip (c->fd)), 0, start, stop);
...
static Transfer *start_upload (FTTransfer *xfer)
{
	...
	user = ft_node_user_host (xfer->ip, alias);

	transfer = FT->upload_start (FT, user, xfer->share_authd,
...
int server_setup_upload (FTTransfer *xfer)
{
	...
	if (xfer->stop == 0)
	{
		assert (xfer->share_authd != NULL);
		xfer->stop = xfer->share_authd->size;
	}
	...
	if (!server_setup_upload (xfer))
	{
		...
		switch (xfer->code)
		{
		 case 503:                     /* send queue position information */
			{
				char *queue_pos   = stringf_dup ("%i", 1);
				char *queue_retry = stringf_dup ("%i", (30 * SECONDS));
...
void server_upload_file (int fd, input_id id, TCPC *c)
{
	...
	if ((remainder = chunk->stop - (chunk->start + chunk->transmit)) <= 0)
	{
		...
		FT->chunk_write (FT, chunk->transfer, chunk, chunk->source, NULL, 0);
		return;
	}
	...
	{
		FT->err (FT, "unable to read from %s: %s",
		         xfer->share_authd->path, GIFT_STRERROR());
	...
	FT->chunk_write (FT, chunk->transfer, chunk, chunk->source, buf, sent_len);
}
