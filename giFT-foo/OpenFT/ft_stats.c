/*
 * $Id: ft_stats.c,v 1.13 2003/06/24 19:57:20 jasta Exp $
 *
...
void ft_stats_insert (in_addr_t depend, in_addr_t user, ft_stats_t *stats)
{
	...
	dataset_insert (&by_user, &user, sizeof (user), stats, sizeof (stats));
}
                  ...
                  double *size, Dataset **extra)
{
	...
	if (stats.users == 0)
	{
		FT->warn (FT, "no stats-provider found, falling back to number of"
		              "established connections (%i)...", conns);
		stats.users = conns;
	}
