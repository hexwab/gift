/*
 * $Id: ft_http_client.c,v 1.29 2003/06/26 09:24:21 jasta Exp $
 *
...
int http_handle_code (FTTransfer *xfer, int code)
{
	...
	switch (code)
	{
	 ...
	 case 500:                     /* source may not match, check later */
		FT->DBGFN (FT, "forcing removal of source %s", chunk->source->url);
		assert (chunk->source->chunk == chunk);
...
void get_read_file (int fd, input_id id, TCPC *c)
{
	 ...
	 * ft_download.
	 */
	FT->chunk_write (FT, chunk->transfer, chunk, chunk->source, buf, recv_len);
}
