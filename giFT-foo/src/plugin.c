/*
 * $Id: plugin.c,v 1.44 2003/06/26 09:24:21 jasta Exp $
 *
...
static int upload_auth_wrapper (Protocol *p, const char *user, Share *share)
{
	return upload_auth (user, share);
}
...
static char *get_primary_hash (Protocol *p)
{
	return dataset_find (p->hashes, DS_FIND(find_primary_hash), NULL);
}

static Transfer *upload_start_wrapper (Protocol *p, const char *user,
                                       Share *share, off_t start, off_t stop)
{
	Transfer *t;
	char     *hash_type;
	char     *hash_dsp;

	hash_type = get_primary_hash (p);
	assert (hash_type != NULL);

	hash_dsp = share_dsp_hash (share, hash_type);
	assert (hash_dsp != NULL);

	/* and now for the ugly internal interface */
	t = upload_new (p, (char *)user, hash_type, hash_dsp,
...
static void source_abort_wrapper (Protocol *p, Transfer *transfer,
                                  Source *source)
{
	Chunk *chunk;
...
static Share *share_lookup_wrapper (Protocol *p, int command, ...)
{
	Share  *share;
	va_list args;
...
# endif /* PLUGIN_GNUTELLA */
	...
	if (!(p = protocol_new (pname, LIBGIFTPROTO_VERSION)))
		return NULL;
