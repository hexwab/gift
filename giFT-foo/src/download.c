/*
 * $Id: download.c,v 1.243 2003/06/09 02:59:55 jasta Exp $
 *
                        ...
                        off_t size)
{
	...
	transfer_log (transfer,
	              ...
	              "  Hash:     %s\n"
	              "  Size:     %i\n",
	              DOWNLOAD(transfer)->uniq, filename, hash, (int) size);
...
static int add_source (Transfer *transfer, Source *source)
{
	...
	if ((old_source = locate_source (transfer, source)))
	{
		...
		GIFT_TRACE (("replacing %s with %s...", old_source->url, source->url));
