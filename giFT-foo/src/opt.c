/*
 * $Id: opt.c,v 1.15 2003/06/26 12:08:48 jasta Exp $
 *
...
static int handle_opt (giftopt_t *opts, int c, int longidx)
{
	 ...
	 * TODO: make sure none of this data could be unaligned */
	switch (opt->type)
	{
	 case OPT_STR:  (*((char **)opt->data)) = optarg;          break;
	 case OPT_LST:  array_push ((Array **)opt->data, optarg);  break;
	 ...
	 case OPT_FLG:  (*((BOOL *)opt->data))  = 1;               break;
...
BOOL opt_parse (int *argc, char ***argv, giftopt_t *opts)
{
	BOOL           ret = TRUE;
#ifdef USE_GETOPT
	char          *optstr;
