/*
 * $Id: http_request.c,v 1.11 2003/06/07 05:45:42 hipnod Exp $
 *
...
static void read_headers (int fd, input_id id, TCPC *c)
{
	...
	{
		GT->DBGFN (GT, "error: no Content-Length str for %s", req->host);
		http_request_close (req, -1);
		return;
	}
...
void handle_http_request (int fd, input_id id, TCPC *c)
{
	HttpRequest *req;
