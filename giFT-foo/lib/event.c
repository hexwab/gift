/*
 * $Id: event.c,v 1.73 2003/06/26 12:08:48 jasta Exp $
 *
 ...
 * WARNING:
 *
 * There is a condition here when timer->cb removes the timer and adds
 * another one.  The possibility of a new timer being created with the same
 * id may cause the newly created tuner to instantly reset or be removed,
 * depending on the return value from the original timer.
 */
static void dispatch_timer (Timer *timer)
{
	...
	/* the timer cb appears to have removed the timer without use of the
	 * return value, bastards */
	if (timer->used == FALSE)
		return;
