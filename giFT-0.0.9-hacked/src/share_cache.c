/*
 * $Id: share_cache.c,v 1.53 2003/06/06 04:06:35 jasta Exp $
 *
...
static int add_share (Dataset **shares, List **duplicates, FileShare *file)
{
	...
	if (!(cache = find_share_hash (shares, NULL, share_get_hash (file, NULL))))
...
void share_write_index (SubprocessData *subproc)
{
	...
	tmp_path = gift_conf_path ("shares.tmp");
	if (!(db = sdb_create (tmp_path, "wb+")))
		return;
	...
	/* duplicate the tmp_path so that the next call to gift_conf_path doesn't
	 * clobber it */
	tmp_path = STRDUP (tmp_path);
...
static void emit_share_new_all (ds_data_t *key, ds_data_t *value, void *udata)
{
	...
	protocol_foreach (DS_FOREACH(emit_share_new), value->data);
}
...
{
	protocol_foreach (DS_FOREACH(emit_share_free), value->data);
}
                                   ...
                                   Dataset *latter)
{
	if (!eq_filesets (key, value, latter))
		protocol_foreach (DS_FOREACH(emit_share_remove), value->data);
}
                               ...
                               Dataset *former)
{
	if (!eq_filesets (key, value, former))
		protocol_foreach (DS_FOREACH(emit_share_add), value->data);
...
static void handle_protocols (Dataset *former, Dataset *latter)
{
	 ...
	 */
	begin = TRUE; protocol_foreach (DS_FOREACH(emit_share_sync), &begin);
	...
	begin = FALSE; protocol_foreach (DS_FOREACH(emit_share_sync), &begin);
