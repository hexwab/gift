/*
 *  $Id: meta_avi.c,v 1.14 2003/06/06 04:06:35 jasta Exp $
 *
...
int meta_avi_run (FileShare *file, char *path)
{
	...
	if (avifile->anum > 0)
	{
		...
		share_set_meta (file, "audioformat", audio_format);
		share_set_meta (file, "audiofrequency",
		                stringf ("%li", audiotrack->a_rate));
#if 0
		share_set_meta (file, "bits per sample",
		                stringf ("%li bits", audiotrack->a_bits));
#endif
		share_set_meta (file, "audiobitrate",
		                stringf ("%li", (long)(audiotrack->mp3rate * 1000)));
	}

	/* extract video information */
	share_set_meta (file, "videoformat", avifile->compressor);
	share_set_meta (file, "resolution",
	                stringf ("%lix%li", avifile->width, avifile->height));
	share_set_meta (file, "fps", stringf ("%4.3f", avifile->fps));
	share_set_meta (file, "duration",
	                stringf ("%li",
	                         (long)((float)avifile->video_frames /
	                                (float)avifile->fps)));
	share_set_meta (file, "bitrate",
	                stringf ("%li",
