/*
 * $Id: if_event.c,v 1.18 2003/06/06 04:06:35 jasta Exp $
 *
...
static int find_and_remove_id (ds_data_t *key, ds_data_t *value, IFEvent *match)
{
	if (find_id (key, value, match))
		return DS_BREAK | DS_REMOVE;
...
static int remove_event (TCPC *c, IFEvent *event)
{
	...
	dataset_foreach_ex (ifc->events, DS_FOREACH_EX(find_and_remove_id), event);
	return TRUE;
}
