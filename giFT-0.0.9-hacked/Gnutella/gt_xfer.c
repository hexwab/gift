/*
 * $Id: gt_xfer.c,v 1.46 2003/06/07 07:13:01 hipnod Exp $
 *
...
char *gt_localize_request (GtTransfer *xfer, char *s_path, int *authorized)
{
	...
	strcpy (open_path, share_get_hpath (file));
...
void gt_push_source_remove (gt_guid_t *guid, in_addr_t ip, in_addr_t src_ip)
{
	...
	list_foreach_remove (src->xfers, (ListForeachFunc)cleanup_xfer, NULL);
	list_foreach_remove (src->connections, (ListForeachFunc)cleanup_conn, NULL);
