/*
 * $Id: dataset.c,v 1.28 2003/05/28 03:39:59 jasta Exp $
 *
...
typedef unsigned long (*HashFunc) (Dataset *d, ds_data_t *key);
...
void ds_data_init (ds_data_t *dsdata, void *data, size_t len, int flags)
{
	assert (dsdata != NULL);
...
static ds_data_t *ds_data_dup (ds_data_t *data)
{
	...
	else
	{
		...
		if (!(dup->data = gift_memdup (data->data, data->len)))
		{
			free (dup);
			return NULL;
		}
...
static ds_data_t *ds_data_reassign (ds_data_t *old, ds_data_t *new)
{
	ds_data_free (old);

	return ds_data_dup (new);
}
...
DatasetNode *dataset_insertstr (Dataset **d, char *key, char *value)
{
	assert (key != NULL);
	assert (value != NULL);

	return dataset_insert (d,
	                       (void *)key, STRLEN_0(key),
	                       (void *)value, STRLEN_0(value));
}
...
{
	return dataset_lookup (d, (void *)key, STRLEN_0(key));
}
...
unsigned int dataset_length (Dataset *d)
{
	...
	switch (d->type)
	{
	 case DATASET_LIST:
		...
		break;
	 case DATASET_ARRAY:
		cnt = (unsigned int)count (&(d->tdata.array));
		break;
	 case DATASET_HASH:
