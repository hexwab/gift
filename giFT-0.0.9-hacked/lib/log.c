/*
 * $Id: log.c,v 1.45 2003/06/06 04:06:35 jasta Exp $
 *
...
void log_print (int priority, char *message)
{
	...
	for (i = 0; i < count (&log_fds); i++)
	{
		FILE *f;

		if (!(f = splice (&log_fds, i, 0, NULL)))
			continue;
...
#if defined(WIN32) && defined(LIBGIFT_DEBUG)
	if (log_options & GLOG_DEBUGGER)
	{
		OutputDebugString (message);
		OutputDebugString ("\n");
	}
#endif /* WIN32 && LIBGIFT_DEBUG */
}
...
void log_fatal (const char *fmt, ...)
{
	...
	log_print (LOG_CRIT,
	           "Consult the log file, or consider using the -v command "
	           "line switch.");
}
